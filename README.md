...
- 🗂️ **Lists overview & Kanban boards** — the home view shows every list side by side, each in its own card with an "Add a task" row and a foldable "Completed" section; the same tasks can be worked as drag-and-drop stacks on a board. Subtasks, comments, labels, assignees, priorities, due dates and recurrence throughout; cards and rows are tinted with the task color for at-a-glance scanning.
- ✍️ **In-place task composer** — "Add a task" opens an editor right where the task will live (title, details, quick "Today"/"Tomorrow" chips, full date-and-time picker, repeat control); it stays open so several tasks can be typed in a row, and a half-typed task is saved rather than lost.
...
- ⚙️ **Admin & per-user settings** — instance-wide policy (board sharing, board/task limits, completed-task retention, attachment size) from the core's admin console; display density, default view and grouping stored per user.
...
Rust 2021 · Axum 0.7 · Tokio · SQLx 0.8 (PostgreSQL 16, schema `tasks`) — React 19 · TypeScript · Vite · Tailwind CSS v4 · Zustand · React Query.
